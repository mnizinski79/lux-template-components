/* ─────────────────────────────────────────────────────────────────────────────
   PRE-SKILL SNAPSHOT (wip) — final/ as of Sep 24, before the review-animations
   recommendations were adopted. Kept only for the before/after compare page.
  ...
  const TRANSITIONS = {
    ...
    drift: {
      ...
      incoming: (dir) => [
        {
          target: 'layer',
          kf: [
            { opacity: 0, transform: `translateX(${dir * 32}px)` },
      ...
      outgoing: (dir) => [
        {
          target: 'layer',
          kf: [{ opacity: 0, transform: `translateX(${-dir * 32}px)` }],
  ...
  function create(opts) {
    ...
    stage.addEventListener('pointermove', (e) => {
      ...
      if (!drag.active) {
        ...
        // Grab the image wherever it is, even mid-transition.
        navSeq++;
        stage.querySelectorAll('.lb__layer.is-leaving').forEach((l) => l.remove());
        freeze(current);
      ...
      if (transition === 'slide' && !reduced()) {
        ...
        drag.peek.style.transform = `translateX(${dx + dir * w}px)`;
      } else {
        // Everything else: the image follows the finger and dims as it goes.
        current.style.transform = `translateX(${dx}px)`;
        current.style.opacity = String(1 - Math.min(Math.abs(dx) / w, 1) * 0.6);
      }
    });
    ...
    function endDrag(e) {
      ...
      const w = travelWidth();
      const s = d.samples;
      const v = s.length > 1 ? (s[s.length - 1].x - s[0].x) / Math.max(s[s.length - 1].t - s[0].t, 1) : 0; // px/ms
      const flick = Math.abs(v) > 0.4 && Math.sign(v) === Math.sign(d.dx);
      ...
      const old = current;
      freeze(old);
      play(old, [{ target: 'layer', kf: [{ transform: `translateX(${d.dx + -dir * 80}px)`, opacity: 0 }], ms: 300, ease: EASE.out }])
        .then(() => old.remove());
    ...
    function onKey(e) {
      ...
      // Held-down arrow keys skip animation entirely: at repeat rate the
      // motion is noise, not information.
      else if (e.key === 'ArrowRight') { e.preventDefault(); next({ instant: e.repeat }); }
      else if (e.key === 'ArrowLeft') { e.preventDefault(); prev({ instant: e.repeat }); }
      else if (e.key === 'Tab') {
        const f = focusables();
