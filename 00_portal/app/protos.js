...
const PROTOS = [
...

  {
    ...
    figma: null,
    motionSpec: null,
    about: 'Live default/hover/focus/active states for the base teaserCard — hover image scale (1.06x) and momentary active overlay are the two animated behaviors under review for v1. A parallel static version (no motion, cursor-only feedback) exists in the Figma spec for comparison.',
    docs: null,
    notes: [
...

  {
    ...
    notes: [
      { date: 'Jul 7', text: 'Desktop accessibility audit — all 4 supplied photos pass AA contrast in both default and hover states (worst case 6.17:1). Purple brand-wash tint checked against the greenest photo (Bali Coastline): measured shift is subtle, not a visible clash. Keyboard/screen-reader gap found and documented as a dev handoff requirement (hover-only reveal, no keyboard path).' },
      ...
      { date: 'Apr 15', text: 'Initial exploration — 3 directions (overlay, carousel, accordion) presented for review.' },
    ],
    urls: {
...

  {
    ...
    about: 'Header (text-only, no icon/meta list) + a WDS Accordion instance, 1440px block width and token-accurate padding/gutter. Toggle switches between the two live headingPosition layouts — above (stacked) and start (header pinned to a 373px column, accordion filling the rest). Body content demonstrates all three content sub-components: textBlock (paragraph + optional link), unorderedList (diamond bullets), and contactStack (icon + phone/email).',
    docs: null,
    notes: [],
    urls: {
...

  {
    ...
    docs: [
      ...
      { label: 'Motion Before / After', url: '../_prototype/wds/mediaGalleryLightbox/wip/lightbox-skill-compare.html', hint: 'Earlier motion compared with the current motion' },
    ],
    notes: [
      { date: 'Sep 24', text: 'Mobile navigation: swipe follows the finger and leaves at the speed it\'s thrown; tapping the left or right half of the image steps back or forward.' },
      { date: 'Sep 24', text: 'Image-to-image motion: a short directional drift (32px, 450ms in / 300ms out) with a light blur where images overlap. Keyboard navigation is instant. Captions and arrows hold their position between items.' },
      { date: 'Sep 24', text: 'Rebuilt to match the final Figma component across Desktop, Tablet, and Mobile, with every value mapped to Regent design tokens. The gallery opens from a 1:1 MediaGalleryGrid.' },
    ],
    urls: {
